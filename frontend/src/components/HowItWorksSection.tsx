  ...
  const steps = [
    {
      ...
      description: "A student fills a simple form with their name, wallet address, required amount, and reason for financial help.",
      features: [
        "Student details submitted",
        "Wallet address added",
        "Amount & purpose mentioned"
      ],
      delay: "delay-200"
    },
    {
      icon: Wallet,
      number: "02",
      title: "Request Reviewed by Donor / Admin",
      description: "Donors or administrators view all student requests, including their stories and required funding.",
      features: [
        "All requests visible",
        "Clear student stories",
    ...
    },
    {
      icon: CreditCard,
      number: "03",
      title: "Funds Sent Directly",
      description: "The donor clicks “Send Money”, connects their wallet, and confirms the transaction.",
      features: [
    ...
    },
    {
      ...
      description: "The transaction is permanently recorded on the blockchain and cannot be changed or deleted.",
      features: [
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className={`text-center mb-16 transition-all duration-700 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>          <div className="flex justify-center mb-6">
          ...
          <h2 className="text-3xl md:text-5xl lg:text-6xl font-medium tracking-tighter text-foreground mb-6">
            <span className="text-cyan-200">From Trust Issues</span>
            <span className="text-white block">To Trust by Technology</span>
          </h2>
          ...
          <p className="hidden md:block text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto text-balance">
            ...
            Every scholarship transaction is direct, transparent, and publicly verifiable.
          </p>
        </div>
        ...
        <div className="block md:hidden relative px-8 mb-12">
              ...
              {steps.map((step, index) => {
                ...
                return (
                  <CarouselItem key={index}>
                    <Card
                      className={`cosmic-glow relative p-6 bg-background/80 backdrop-blur-sm border-border hover:shadow-lg transition-all duration-700 group w-full h-96 sm:h-[28rem] ${isVisible ? `opacity-100 translate-y-0 ${step.delay}` : 'opacity-0 translate-y-10'
                        }`}
                    >
          ...
          <div className="flex justify-center gap-1.5 mt-4">
            {steps.map((_, idx) => (
              <button
                key={idx}
                className={`w-2 h-2 rounded-full transition-colors duration-200 ${selectedIndex === idx
                  ? 'bg-primary'
        ...
        <div className="hidden md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
          {steps.map((step, index) => {
            ...
            return (
              <div
                key={index}
                className={`relative transition-all duration-700 transform ${isVisible ? `opacity-100 translate-y-0 ${step.delay}` : 'opacity-0 translate-y-10'
                  }`}
              >
                {/* Connection Line */}
        ...
        <div className={`cosmic-glow relative rounded-xl overflow-hidden border border-border backdrop-blur-sm bg-background/50 shadow-lg transition-all duration-700 delay-1000 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="p-8 text-center">
            ...
            <p className="text-muted-foreground mb-6 max-w-2xl mx-auto">
              Donors and students can both see proof that funds were sent and received —
              building trust across borders.
            </p>

            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button
                className="bg-primary text-primary-foreground hover:bg-primary/80 hover:text-primary-foreground text-base h-12 px-8 transition-all duration-200"
