...
const WalletOnboarding: React.FC<WalletOnboardingProps> = ({ onComplete }) => {
  ...
  // EVM wallet generation using ethers
  const generateWallet = async (): Promise<WalletData> => {
    try {
      const walletData = createWallet();

      // Fund the new account with testnet ETH (from Hardhat/Localhost faucet)
      const funded = await fundWallet(walletData.address);
      if (funded) {
        console.log('Account funded with testnet ETH');
        toast({
          title: "Wallet Funded",
          description: "Your new wallet has been funded with 10 ETH from the local faucet!"
        });
      }
  ...
  const handleImportWallet = async () => {
    ...
    try {
      ...
      if (importType === 'mnemonic' && importMnemonic.trim()) {
        ...
        // Removed isValidMnemonic check as it was Aptos specific (ethers will validate)
        ...
        if (!importPrivateKey.startsWith('0x') || importPrivateKey.length !== 66) {
          throw new Error('Invalid private key format (must start with 0x and be 66 chars)');
        }
  ...
  const renderWelcomeStep = () => (
    <Card className="w-full max-w-md mx-auto bg-secondary/50 border-border/50 backdrop-blur-sm relative z-10">
      <CardHeader className="text-center space-y-4">
        ...
        <CardTitle className="text-2xl text-foreground">Welcome to Wallet</CardTitle>
  ...
  const renderCompleteStep = () => (
    <Card className="w-full max-w-md mx-auto bg-secondary/50 border-border/50 backdrop-blur-sm relative z-10">
      <CardHeader className="text-center space-y-4">
        ...
        <CardDescription className="text-muted-foreground">
          Your wallet has been {onboardingType === 'create' ? 'created' : 'imported'} successfully
        </CardDescription>
      </CardHeader>
        ...
        <div className="space-y-4">
          ...
          <Alert className="bg-secondary/30 border-border/30">
            ...
            <AlertDescription className="text-muted-foreground">
              ...
              {onboardingType === 'create' && ' It has been connected to Local Ethereum Network and funded with 10 ETH for testing.'}
            </AlertDescription>
          </Alert>
        </div>
            ...
            <div className="flex items-center justify-center space-x-2 mb-4">
              {steps.map((step, index) => (
                <div key={index} className="flex items-center">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium border ${index <= currentStep
                  ...
                  {index < steps.length - 1 && (
                    <div className={`w-8 h-0.5 mx-2 ${index < currentStep ? 'bg-primary' : 'bg-border/30'
                      }`} />
                  )}
                </div>
              ))}
